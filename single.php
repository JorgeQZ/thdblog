...
<div class="container">
    ...
    <?php
        ...
        if ($img_html === '') {
            $img_html = sprintf(
                '<img class="post-featured-image__img" src="%s" alt="%s" loading="lazy" decoding="async" width="1024" height="529">',
    ...
    <div class="post-featured-image">
        ...
        echo $img_html; // aquí va la imagen (attachment o fallback)
        echo '</figure>';
        ?>
    </div>
    ...
    <div class="grid">
        ...
        $mostrar_d_y_d = function_exists('get_field') ? get_field('_mostrar_d_y_d', $id) : get_post_meta($id, '_mostrar_d_y_d', true);
        $duracion       = is_array($duracion_raw) ? '' : trim((string)$duracion_raw);
        $dificultad     = is_array($dificultad_raw) ? '' : trim((string)$dificultad_raw);


        if ($mostrar_d_y_d !== 'No'): // mostrar
            ?>
        <div class="column">
            <?php if ($duracion !== ''): ?>
        ...
        if (!is_wp_error($terms) && !empty($terms)) {
            ...
            if ($q->have_posts()) {
                foreach ($q->posts as $pid2) {
                    ...
                    if (is_array($t2)) {
                        foreach ($t2 as $t) {
                            if (in_array($t->term_id, $seed_ids, true)) {
                                continue;
                            }
            ...
            if (!empty($related_terms)) : ?>
                <div class="grid">
                    <?php foreach ($related_terms as $term) :
                        ...
                        ?>
                    <a class="item" href="<?php echo esc_url($term_link); ?>"
